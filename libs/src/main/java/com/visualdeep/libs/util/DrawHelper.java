...
public class DrawHelper {
    ...
    public void draw(FaceRectView faceRectView, List<DrawInfo> drawInfoList) {
        ...
        for (DrawInfo drawInfo : drawInfoList) {
            Rect sRect = drawInfo.getRect();
            Rect tRect = adjustRect( sRect , previewWidth, previewHeight, canvasWidth, canvasHeight, cameraDisplayOrientation, cameraId,
                    isMirror, false, false);
            drawInfo.setRect( tRect );
        }
        faceRectView.addFaceInfo(drawInfoList);
    }

  public void draw(FaceRectView faceRectView, FaceDetector.Face[] faces) {
    if (faceRectView == null) {
      return;
    }
    faceRectView.clearFaceInfo();
    ...
    for ( FaceDetector.Face face  : faces ) {
      ...
      if (face != null) {
        face.getMidPoint(pf);
        Rect sRect = new Rect();
        sRect.left = (int) (pf.x - face.eyesDistance() / 2);
        sRect.right = (int) (pf.x + face.eyesDistance() / 2);
        sRect.top = (int) (pf.y - face.eyesDistance() / 2);
        sRect.bottom = (int) (pf.y + face.eyesDistance() / 2);
        Rect tRect = adjustRect(sRect, previewWidth, previewHeight, canvasWidth, canvasHeight, cameraDisplayOrientation, cameraId,
          isMirror, false, false);

        DrawInfo drawInfo = new DrawInfo(tRect, "");
